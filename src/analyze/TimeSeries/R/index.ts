export function movingAverages(arr: number[], periods: number): number[] {
  const response = new Array(periods).fill(0);
...
export function weightMovingAverages(
  ...
  const response = new Array(periods.length).fill(0);
  const weightSum = periods.reduce((pre, curr) => pre + curr);
...
export function exponentialSmoothing(
  ...
  const forecasts = [PreviousForecast];
  for (let i = 1; i <= arr.length; i++) {
    const Sum = PreviousForecast + alpha * (arr[i - 1] - PreviousForecast);
